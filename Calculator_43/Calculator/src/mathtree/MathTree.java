...
{
   private mathTree.Expression rootNode = null;
   private StringScanner strScanner = new StringScanner(); //Set up in the constructor
   private mathTree.Factory nodeFactory = new mathTree.Factory();
    ...
    */
   private mathTree.Expression buildTree(LinkedList<String> strTokens, boolean isParens)
   {
      String token;
      mathTree.Expression rootNode = null;
      //mathNode.Expression lastNode = null;
      mathTree.Expression newNode = null;
    ...
    */
   private mathTree.Expression insertNode(mathTree.Expression rootNode, 
         mathTree.Expression newNode)
   {
      ...
      else if(newNode instanceof mathTree.Operator && !newNode.isParens())
      {
         mathTree.Operator newOperator = (mathTree.Operator) newNode;
         mathTree.Operator parent;
         
         //Check if rootNode is an operator
         if(rootNode instanceof mathTree.Operator)
            parent = (mathTree.Operator) rootNode;
         else
         {
         ...
         while(parent.getPrecedence() > newOperator.getPrecedence() 
               ...
               && parent.getRightNode() instanceof mathTree.Operator)
         {
            parent = (mathTree.Operator) parent.getRightNode();
         }
      ...
      else
      {
         ...
         {
            if(parent.getRightNode() instanceof mathTree.Operator)
               parent = (mathTree.Operator) parent.getRightNode();
            else
            {
               System.out.println("Invalid: Missing operator between " + 
   ...
   public Object clone() throws CloneNotSupportedException
   {
      ...
      clone.nodeFactory = (mathTree.Factory) nodeFactory.clone();
      clone.rootNode = (mathTree.Expression) rootNode.clone();
